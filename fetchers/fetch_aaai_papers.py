...
CONF_LINKS = ['AAAI/aaai22contents.php',
              'AAAI/aaai21contents.php',
...
CONF_NAMES = ['AAAI2022', 'AAAI2021', 'AAAI2020', 'AAAI2019', 'AAAI2018', 'AAAI2017', 'AAAI2016', 'AAAI2015']
...
def fetch_papers(db_manager: DBManager,
    ...
    if conf_year in [2022, 2021]:
        contents_list = soup.find_all('a')
        issues_links = [c.get('href') for c in contents_list]
        issues_links = [i for i in issues_links if i.startswith(f'aaai{str(conf_year)[-2:]}-issue') and not '#' in i]
        ...
        if (len(page_urls_list) == len(authors_list) and
            ...
            for i, page_url in enumerate(tqdm(page_urls_list)):
                ...
                if not db_manager.exists(pid):
                    try:
                        if not conf_year in [2022, 2021, 2020, 2019]:
