...
def main() -> None:
    ...
    fetch_papers(db_manager, base_url, list_url, 'ICML2020', 'Main', 'ICML2020')
    db_manager.write_db()
...
def fetch_papers(db_manager: DBManager,
    ...
    papers_meta_list = soup.find_all('div', {'class': 'myCard col-sm-6 col-lg-4'})
    ...
    conf_year = conf_id[-4:]
    conf_date = dateutil.parser.parse(conf_year + '-07')
        ...
        for i in tqdm(range(len(titles_list))):
            ...
            if not db_manager.exists(pid):
                ...
                try:
                    ...
                    pdf_url = soup2.find_all('a', {'class': 'card-link'})
                    pdf_url = [p.get('href') for p in pdf_url]
                    pdf_url = [p for p in pdf_url if '-Paper.pdf' in p][0]
                    summary = flatten_content_list(soup2.find('div', {'id': 'abstractExample'}).contents)
                    print(pdf_url)
                    print(summary)
